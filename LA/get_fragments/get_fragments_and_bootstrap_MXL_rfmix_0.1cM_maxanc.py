...
target_panel=np.loadtxt("/rds/general/project/human-popgen-datasets/live/HGDP_1000g/RFMix/inds_target_rename_rfmix1.5.4.txt",delimiter='_',dtype="str")
all_inds=target_panel

inds_1=np.where(all_inds[:,1]=="MXL")[0]*6
inds_2=np.where(all_inds[:,1]=="MXL")[0]*6+1
inds_3=np.where(all_inds[:,1]=="MXL")[0]*6+2
inds_4=np.where(all_inds[:,1]=="MXL")[0]*6+3
inds_5=np.where(all_inds[:,1]=="MXL")[0]*6+4
inds_6=np.where(all_inds[:,1]=="MXL")[0]*6+5
indices=np.sort(np.concatenate([inds_1,inds_2,inds_3,inds_4,inds_5,inds_6]))

with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/RFMIX_1.5.4_01cM_"+str(chr), 'rb') as f:    
    rfmix_chr_list=pickle.load(f)

rfmix_chr=np.concatenate(rfmix_chr_list,axis=1)
pos_chr=np.loadtxt("/rds/general/project/human-popgen-datasets/live/HGDP_1000g/RFMix/snps_pos_rfmix_short_B_"+str(chr))
...
with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/lengths_counts_rfmix_maxanc_MXL_chr_"+str(chr), 'wb') as f: 
    pickle.dump(lengths_counts,f)


with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/length_anc_rfmix_MXL_maxanc_chr_"+str(chr), 'wb') as f:
