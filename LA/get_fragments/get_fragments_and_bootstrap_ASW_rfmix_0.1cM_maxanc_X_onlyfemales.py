...
target_panel=np.loadtxt("/rds/general/project/human-popgen-datasets/live/HGDP_1000g/RFMix/targetAFRAME_panel.txt",delimiter='\t',dtype="str")
all_inds=target_panel

rfmix_chr=np.loadtxt("/rds/general/project/human-popgen-datasets/live/HGDP_1000g/RFMix/HGDP_1000gNYC_3pop_target_ref_output_rfmix154_short_B_X_AFRAME.1.ForwardBackward.txt")
females_1=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="F"))[0]*6
females_2=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="F"))[0]*6+1
females_3=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="F"))[0]*6+2
females_4=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="F"))[0]*6+3
females_5=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="F"))[0]*6+4
females_6=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="F"))[0]*6+5
males_1=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="M"))[0]*6
males_2=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="M"))[0]*6+1
males_3=np.where((all_inds[:,1]=="ASW")&(all_inds[:,2]=="M"))[0]*6+2
indices=np.sort(np.concatenate([females_1,females_2,females_3,females_4,females_5,females_6]))
pos_chr=np.loadtxt("/rds/general/project/human-popgen-datasets/live/HGDP_1000g/RFMix/snps_pos_rfmix_short_B_X_AFRAME")
...
with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/lengths_counts_rfmix_maxanc_ASW_chr_X_onlyfemales", 'wb') as f: 
    pickle.dump(lengths_counts,f)

with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/length_anc_rfmix_ASW_maxanc_chr_X_onlyfemales", 'wb') as f:
