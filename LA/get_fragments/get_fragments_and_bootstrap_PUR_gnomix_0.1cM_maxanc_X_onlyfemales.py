...
females_1=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="F"))[0]*6
females_2=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="F"))[0]*6+1
females_3=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="F"))[0]*6+2
females_4=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="F"))[0]*6+3
females_5=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="F"))[0]*6+4
females_6=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="F"))[0]*6+5
males_1=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="M"))[0]*6
males_2=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="M"))[0]*6+1
males_3=np.where((all_inds[:,1]=="PUR")&(all_inds[:,2]=="M"))[0]*6+2
indices=np.sort(np.concatenate([females_1,females_2,females_3,females_4,females_5,females_6]))

filename="/rds/general/project/human-popgen-datasets/live/HGDP_1000g/gnomix/query_results_0.1cM_X.fb"
...
with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/lengths_counts_gnomix_maxanc_PUR_chr_X_onlyfemales", 'wb') as f:
    pickle.dump(lengths_counts,f)

with open("/rds/general/user/amassand/home/SLIM_models/MODELS/pops/length_anc_gnomix_PUR_maxanc_chr_X_onlyfemales", 'wb') as f:
