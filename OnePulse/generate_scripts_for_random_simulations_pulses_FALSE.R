...
for (pop in 1:length(colnames(pops))){
	...
	AMcomb_pop_file<-paste(path,"AM_combinations_pulses_false_scale_",scale,"_pop_",pop,".txt",sep="")
...
system(paste("rm ",path,"runrunfile_slimAutX_pulses_false_scale_10E3.sh",sep=""))
for (SCALE in scale) {
...
for (pop in 1:length(colnames(pops))){  
      ...
      output_jobfile <- paste(path,colnames(pops)[pop],"/Slim_output_AutX_pulses_false_","_GEN_",as.character(GEN),"_POP1_",as.character(RATPOP1),"_POP2_",as.character(RATPOP2),"_totalpop_",as.character(TOTALPOP),"_pop_",pop,"_",colnames(pops)[pop],"_scale_",SCALE,"_run_$(($PBS_ARRAY_INDEX+",lastrun_pop[pop],")).txt",sep="")
      fragsfile <- paste(path,colnames(pops)[pop],"/fragments/Frags_AutX_pulses_false_","_GEN_",as.character(GEN),"_POP1_",as.character(RATPOP1),"_POP2_",as.character(RATPOP2),"_totalpop_",as.character(TOTALPOP),"_pop_",pop,"_",colnames(pops)[pop],"_scale_",SCALE,"_run_$(($PBS_ARRAY_INDEX+",lastrun_pop[pop],")).txt",sep="")
      ancfile <- paste(path,colnames(pops)[pop],"/ancestries/Ancestries_AutX_pulses_false_","_GEN_",as.character(GEN),"_POP1_",as.character(RATPOP1),"_POP2_",as.character(RATPOP2),"_totalpop_",as.character(TOTALPOP),"_pop_",pop,"_",colnames(pops)[pop],"_scale_",SCALE,"_run_$(($PBS_ARRAY_INDEX+",lastrun_pop[pop],")).txt",sep="")
      ...
      runjobfile <-paste(path,colnames(pops)[pop],"/run_job_AutX_pulses_false","_GEN_",as.character(GEN),"_POP1_",as.character(RATPOP1),"_POP2_",as.character(RATPOP2),"_totalpop_",as.character(TOTALPOP),"_pop_",pop,"_",colnames(pops)[pop],"_scale_",SCALE,".sh",sep="")
      jobname <- paste("job_pulses_false","_GEN_",as.character(GEN),"_POP1_",as.character(RATPOP1),"_POP2_",as.character(RATPOP2),"_totalpop_",as.character(TOTALPOP),"_pop_",pop,"_",colnames(pops)[pop],"_scale_",SCALE,sep="")
      ...
      write(systemcommandNJOBS,paste(path,"runrunfile_slimAutX_pulses_false_scale_10E3.sh",sep=""),append=TRUE)
      ##run both qsub commands one after the other
      write(qsubcommand,paste(path,"runrunfile_slimAutX_pulses_false_scale_10E3.sh",sep=""),append=TRUE)
      system(paste("chmod 770 ",path,"runrunfile_slimAutX_pulses_false_scale_10E3.sh",sep=""))
    }
}}
